...
| 七 | 精炼器（Refiner） |
| 八 | 生成器（OpenAI API / 本地 HF / PromptTemplate） |
| 九 | Pipeline 实战（Naive / FLARE / 自定义） |
| 十 | 结果评估 |
| **十一** | **完整端到端流程（一键跑通）** |
| 十二 | 总结 |
...
用户问题 → ③ retriever.batch_search(queries)    批量检索（实际方法名）
            ...
            ④ reranker.rerank(queries, docs)     CrossReranker（实际类名）
               返回 (reranked_docs, scores) 元组
                ↓
            ⑤ refiner.batch_run(dataset)         Refiner（可选，压缩/精炼检索文档）
               返回 refined_texts: List[str]
                ↓
...
| 精炼器 | `flashrag.refiner` | `ExtractiveRefiner` / `AbstractiveRefiner` / `LLMLinguaRefiner` |
...
### 4.4 Chonkie — 语义分块（Semantic Chunking）

> FlashRAG 集成了 [Chonkie](https://github.com/chonkie-ai/chonkie) 语义分块库，
> 作为 `RecursiveCharacterTextSplitter` 固定长度分块的语义感知替代方案。
> **安装**：`pip install chonkie sentence-transformers`

`SemanticChunker` 用嵌入模型计算相邻句子的语义相似度，在语义边界处切分，
保证每个 chunk 内部主题连贯。与固定长度分块对比：

| 对比维度 | `RecursiveCharacterTextSplitter` | `SemanticChunker`（Chonkie） |
|---------|--------------------------------|-----------------------------|
| 切分依据 | 固定字符数 + 分隔符优先级 | 相邻句子语义相似度 |
| chunk 大小 | 均匀可控 | 不均匀（随语义边界变化） |
| 语义完整性 | 可能在语义中间切断 | 在语义边界处切断 |
| 速度 | 极快（无需编码） | 较慢（需编码每个句子） |
| 依赖 | 仅 `langchain_text_splitters` | `chonkie` + 嵌入模型 |
| 适用场景 | 大规模语料，对速度敏感 | 中小规模语料，追求 chunk 质量 |


```python
# ============================================================
# Chonkie SemanticChunker — 语义感知分块
# 安装依赖（首次运行）：pip install chonkie sentence-transformers
# ============================================================
from chonkie import SemanticChunker  # 语义分块器，按嵌入相似度在语义边界处切分
import json                           # JSON 序列化模块

# SemanticChunker(embedding_model, chunk_size, threshold) :
#   embedding_model : str，嵌入模型路径或 HuggingFace Hub ID
#     推荐轻量模型  → minishlab/potion-base-8M（多语言，~100 MB）
#     中文优先      → BAAI/bge-small-zh-v1.5（~24 MB，中文效果更好）
#   chunk_size      : int，每块最大 token 数（超出后强制分块）
#   threshold       : float，语义相似度阈值（0~1）
#     越低（如 0.3）→ 相邻句子更容易被判断为不同主题 → 块更小、边界更清晰
#     越高（如 0.8）→ 相邻句子更倾向于合并 → 块更大
semantic_chunker = SemanticChunker(       # SemanticChunker: 语义分块器实例
    embedding_model="BAAI/bge-small-zh-v1.5",  # str: 中文嵌入模型（复用检索编码模型）
    chunk_size=512,                        # int: 每块最大 token 数
    threshold=0.5,                         # float: 语义相似度阈值（0.5 = 中等切分粒度）
)

# ---- 演示：对比固定长度分块 vs 语义分块 ----
demo_text = raw_texts[0]["content"]                       # str: 取第一篇文档演示

# RecursiveCharacterTextSplitter 固定长度分块（text_splitter 在 4.1 节定义）
fixed_chunks = text_splitter.split_text(demo_text)        # List[str]: 固定长度分块结果
# SemanticChunker.chunk(text) : 语义分块
#   参数 : str，待分块的文本
#   返回 : List[Chunk]，每个 Chunk 含以下属性：
#     .text         : str，块文本内容
#     .token_count  : int，块 token 数量
#     .start_index  : int，在原文中的起始字符位置
#     .end_index    : int，在原文中的结束字符位置
semantic_chunk_objs = semantic_chunker.chunk(demo_text)   # List[Chunk]: 语义分块结果

print(f"固定长度分块数: {len(fixed_chunks)}")              # 打印固定长度分块数
print(f"语义分块数    : {len(semantic_chunk_objs)}")        # 打印语义分块数
print("\n语义分块预览:")                                   # 打印语义分块预览标题
for i, chunk in enumerate(semantic_chunk_objs, 1):        # 遍历每个语义块
    # chunk.text: str 块文本；chunk.token_count: int token 数
    print(f"  块 {i}: {chunk.token_count:3d} tokens | {chunk.text[:60]}...")  # 打印块大小和文本前缀
```


```python
# ============================================================
# 用 Chonkie 语义分块写入 FlashRAG JSONL 语料
# ============================================================
semantic_corpus_path = f"{CORPUS_DIR}/semantic_corpus.jsonl"  # str: 语义分块语料文件路径
total_semantic = 0   # int: 语义块计数器

with open(semantic_corpus_path, "w", encoding="utf-8") as f:   # 打开输出文件
    for doc_idx, doc in enumerate(raw_texts):                   # 遍历原始文档
        # semantic_chunker.chunk(text) : 返回 List[Chunk]
        s_chunks = semantic_chunker.chunk(doc["content"])        # List[Chunk]: 当前文档的语义分块结果
        for chunk in s_chunks:                                   # 遍历每个语义块
            if not chunk.text.strip():                           # 跳过空白块
                continue                                        # 不写入空块
            record = {                                           # Dict: 单条 JSONL 记录（符合 FlashRAG 格式）
                "id":       f"{doc_idx}_{total_semantic}",       # str: 全局唯一 id
                "title":    doc.get("title", ""),                # str: 文档标题
                "contents": chunk.text,                          # str: 语义块文本（必填字段）
            }
            f.write(json.dumps(record, ensure_ascii=False) + "\n")  # 写入一行 JSON
            total_semantic += 1   # 计数加 1

print(f"语义分块语料已写入: {semantic_corpus_path}")          # 打印语料路径
print(f"原始文档: {len(raw_texts)} 篇  →  语义 chunk 总数: {total_semantic}")  # 打印分块统计
# ---- 后续步骤与固定长度分块完全相同 ----
# 只需将 corpus_path 替换为 semantic_corpus_path，再执行第四章 4.2 节构建索引即可
print("\n后续：将 corpus_path 替换为 semantic_corpus_path，重新运行 4.2 节构建索引")  # 提示后续操作
```
...
### 5.4 WebSearchRetriever — 网络搜索检索

`WebSearchRetriever` 直接向搜索引擎发起查询，获取实时网页摘要作为检索文档，
**无需预先构建本地索引**，适合需要最新信息的问答场景（如 CRAG 算法）。

> **注意**：
> - 需要申请搜索引擎 API Key（Bing / Google / Tavily 等）
> - 需要安装对应 SDK：`pip install tavily-python` 或 `pip install google-api-python-client`
> - 网络延迟较高，每次检索需要真实网络请求，不适合离线评测场景


```python
# ============================================================
# WebSearchRetriever — 实时网络搜索检索
# 官方模块路径：flashrag.retriever
# ============================================================
from flashrag.retriever import get_retriever   # 检索器工厂函数（根据 retrieval_method 自动选择）

# WebSearchRetriever Config 关键字段（来自源码）：
#   retrieval_method   : "bm25"（本地检索）← 若要启用网络搜索，使用特定方法名
#   注意：FlashRAG 的网络搜索检索器通过 retrieval_method 值区分，
#         常见值有 "web" 或特定搜索引擎名称（如 "bing"、"tavily"）
#         具体值以当前安装版本 flashrag/retriever/__init__.py 中的注册表为准
#
# ---- 以 Tavily 为例（免费额度，中文支持较好）----
# 安装: pip install tavily-python
# 申请 API Key: https://tavily.com
# 环境变量: export TAVILY_API_KEY=tvly-xxxx
config_web = Config(config_path, {             # Config: 网络搜索检索器配置
    "retrieval_method":   "web",               # str: 启用网络搜索检索（具体值参考当前版本源码）
    "retrieval_topk":     5,                   # int: 返回 Top-5 搜索结果
    # 以下字段为示意，实际 key 名称以源码为准
    "web_search_engine":  "tavily",            # str: 搜索引擎类型（"bing" / "tavily" 等）
    "web_search_api_key": os.environ.get("TAVILY_API_KEY", "your-tavily-key"),  # str: API Key
})

# get_retriever(config_web) : 根据 retrieval_method 自动实例化对应检索器
# 当 retrieval_method="web" 时，返回 WebSearchRetriever 实例
# 使用方式与 DenseRetriever / BM25Retriever 完全相同
# web_retriever = get_retriever(config_web)   # WebSearchRetriever: 网络搜索检索器实例

# 批量搜索（与本地检索器接口一致）
# web_results = web_retriever.batch_search(["2024 年 RAG 最新进展"])
# 返回 List[List[Dict]]，每个 Dict 含：
#   "contents" : str，网页摘要
#   "title"    : str，网页标题
#   "url"      : str，网页来源 URL

print("WebSearchRetriever 用法与 DenseRetriever 完全一致（batch_search 接口相同）")  # 打印接口说明
print("⚠ 需要搜索引擎 API Key 并联网，取消注释后替换 API Key 即可运行")            # 打印注意事项
```
...
FlashRAG 精炼器（Refiner）位于**检索/重排序之后、生成之前**，
对召回的文档进行压缩、抽取或改写，去除冗余噪声、降低传入生成器的 token 开销，同时尽量保留回答所需的关键信息。

精炼器位于 `flashrag.refiner` 模块；`get_refiner(config)` 工厂函数
根据 `config["refiner_name"]` 自动实例化对应子类（推荐使用）。

主要精炼器类型（来自 `flashrag/refiner/` 源码）：

| 类名 | `refiner_name` | 压缩类型 | 核心依赖 |
|------|---------------|---------|---------|
| `ExtractiveRefiner` | `"extractive"` | 句子级抽取 | 复用检索编码模型，无需额外加载 |
| `AbstractiveRefiner` | `"abstractive"` | 生成式摘要 | 独立 Seq2Seq 模型（如 Flan-T5） |
| `LLMLinguaRefiner` | `"llmlingua"` | Token 级压缩 | `llmlingua` 库 + 小型 LM |
| `SelectiveContextRefiner` | `"selective-context"` | 基于困惑度选择 | GPT-2 类型小型 LM（⚠ 仅支持英文）|
| `BGMRefiner` | `"bgm"` | 生成式精炼 | BGM 专用模型 |

> **何时需要 Refiner：**
> - Top-K 文档总 token 量接近或超过 `generator_max_input_len`（上下文窗口紧张）
> - 使用按 token 计费的 API，希望通过压缩降低调用成本
> - 召回文档含大量噪声/无关段落，精炼后送生成器可提升答案质量
>
> **不需要 Refiner 的常见情形：**
> - 语料 chunk 本身较短（≤200 字），Top-K 文档总量已在生成器窗口内
> - 使用长上下文生成模型（如 Qwen-Long），上下文容量充足

**Refiner 在数据流中的位置：**
```
retriever.batch_search()
    → reranker.rerank()                      （可选）
    → refiner.batch_run(dataset)             ← 本章重点，返回 List[str]
    → prompt_template.get_string(
          question=q,
          formatted_reference=refined_text)  ← 传精炼后字符串而非 List[Dict]
    → generator.generate()
```

### 7.1 Refiner 相关 Config 字段


```python
# ============================================================
# Refiner 相关 Config 字段（来自 basic_config.yaml 和源码）
# ============================================================
# refiner_name           : str|None，精炼器类别名（小写）
#   None / 未设置         → 不启用精炼器
#   "extractive"          → ExtractiveRefiner（抽取式句子筛选，推荐入门）
#   "abstractive"         → AbstractiveRefiner（生成式摘要，需独立模型）
#   "llmlingua"           → LLMLinguaRefiner（token 级压缩，需 llmlingua 库）
#   "selective-context"   → SelectiveContextRefiner（基于困惑度，⚠ 仅支持英文，需 GPT-2 模型）
#   "bgm"                 → BGMRefiner（BGM 专用模型）
# refiner_model_path     : str|None，精炼器模型路径（model-based 精炼器使用）
#   ExtractiveRefiner     → 复用检索编码模型路径（如 BAAI/bge-small-zh-v1.5）
#   AbstractiveRefiner    → 独立 Seq2Seq 模型路径（如 fangyuan/ance_abstractive_compressor）
#   LLMLinguaRefiner      → LLMLingua 模型路径（如 microsoft/llmlingua-2-bert-base-...）
# refiner_input_type     : str，传给精炼器的输入类型
#   "retrieval_results"（默认）→ 使用 dataset.retrieval_result（检索文档列表）作为输入
# refiner_topk           : int，ExtractiveRefiner 保留的 Top-K 句子数
# refiner_encode_max_length : int，精炼器编码模型的最大 token 数（与模型窗口对齐）
# ============================================================
# 示例：仅演示字段含义，不执行实际初始化
refiner_config_example = {   # Dict: Refiner 相关配置字段示例（非完整 Config，仅用于说明）
    "refiner_name":              "extractive",            # str: 启用抽取式精炼器
    "refiner_model_path":        "BAAI/bge-small-zh-v1.5",  # str: 复用检索编码模型
    "refiner_input_type":        "retrieval_results",     # str: 使用检索结果作为输入
    "refiner_topk":              5,                        # int: 每条查询保留 Top-5 相关句
    "refiner_encode_max_length": 512,                      # int: 编码最大 token 数
}
print("Refiner 配置字段说明已加载")   # 打印说明加载成功提示
```

### 7.2 ExtractiveRefiner — 抽取式句子筛选（推荐入门）

`ExtractiveRefiner` **复用检索编码模型**对文档中的每个句子与 query 计算相似度，
保留相似度最高的 Top-K 句子并拼接返回。无需加载额外模型，资源消耗最低，
适合快速降低 token 开销、过滤无关句子。


```python
# ============================================================
# ExtractiveRefiner — 复用编码模型按句子相似度抽取 Top-K 句
# ============================================================
from flashrag.refiner import ExtractiveRefiner  # 抽取式精炼器（按句子相似度筛选 Top-K 句）
from flashrag.utils import get_refiner           # 精炼器工厂函数，根据 refiner_name 自动选择类

# ---- 初始化 ExtractiveRefiner ----
# Config 关键字段：
#   refiner_name             : "extractive"，触发 ExtractiveRefiner
#   refiner_model_path       : 编码模型路径（复用检索编码模型即可）
#   refiner_topk             : int，每条查询保留的 Top-K 相关句子数
#   refiner_encode_max_length: int，编码最大 token 数（与模型窗口对齐）
config_extractor = Config(config_path, {         # Config: 抽取式精炼器专用配置
    "index_path":                actual_index_path,         # str: FAISS 索引文件路径（已构建）
    "refiner_name":              "extractive",              # str: 启用抽取式精炼器
    "refiner_model_path":        "BAAI/bge-small-zh-v1.5",  # str: 复用检索编码模型路径
    "refiner_input_type":        "retrieval_results",       # str: 以检索文档为精炼输入
    "refiner_topk":              5,                          # int: 每条查询保留 Top-5 相关句
    "refiner_encode_max_length": 512,                        # int: 编码最大 token 数
})

# ExtractiveRefiner(config) : 初始化抽取式精炼器
# 内部加载编码模型，对每个 (query, 检索文档) 对计算句子级相似度
extractor = ExtractiveRefiner(config_extractor)   # ExtractiveRefiner: 抽取式精炼器实例
# 等价写法：extractor = get_refiner(config_extractor)  # 工厂函数根据 refiner_name 自动选择

print("ExtractiveRefiner 初始化完成！")                               # 打印初始化成功提示
print(f"  编码模型: {config_extractor['refiner_model_path']}")        # 打印编码模型路径
print(f"  保留 Top-K 句: {config_extractor['refiner_topk']}")         # 打印保留句子数
```


```python
# ============================================================
# 使用 ExtractiveRefiner 精炼检索结果
# ============================================================

# ---- Step 1: 检索候选文档（需先完成第四章 DenseRetriever 初始化）----
# batch_search 返回 List[List[Dict]]，shape: (N, retrieval_topk)
refine_queries = [                       # List[str]: 待精炼查询列表，长度 N=2
    "RAG 技术的三个发展阶段",             # str: 查询 1
    "FlashRAG 支持哪些生成器",            # str: 查询 2
]
# DenseRetriever(config_extractor) : 使用精炼器配置初始化检索器
dense_r = DenseRetriever(config_extractor)              # DenseRetriever: 加载已有 FAISS 索引
refine_candidates = dense_r.batch_search(refine_queries)  # List[List[Dict]]: 粗排候选文档

# ---- Step 2: 将检索结果写入 Dataset，供 Refiner 读取 ----
# refiner.batch_run 需要 dataset.question 和 dataset.retrieval_result 两个字段
# 此处复用前面构造的 test_dataset（需先运行第九章「9.1 准备 Pipeline 输入数据集」小节）
test_dataset.update_output("retrieval_result", refine_candidates)  # 写入检索结果字段

# ---- Step 3: 运行精炼器 ----
# refiner.batch_run(dataset) :
#   参数 : Dataset，需包含 question（查询文本）和 retrieval_result（检索文档列表）字段
#   返回 : List[str]，长度 N
#     每条为精炼后的上下文字符串（ExtractiveRefiner 将文档拆句后按相似度取 Top-K 句拼接）
#     可直接作为 formatted_reference 传给 PromptTemplate.get_string()，跳过自动格式化
refined_texts = extractor.batch_run(test_dataset)  # List[str]: 每条查询精炼后的上下文，长度 N

print(f"精炼前每条查询候选文档数: {[len(d) for d in refine_candidates]}")  # 打印精炼前文档数
print(f"\n精炼后文本（抽取 Top-K 相关句）:")   # 打印精炼结果标题
# 遍历查询和精炼文本，q 为 str，refined 为精炼后上下文字符串
for q, refined in zip(refine_queries, refined_texts):   # q: str 查询，refined: str 精炼后上下文
    print(f"\n  查询: {q}")                               # 打印当前查询文本
    print(f"  精炼后（前 120 字）: {refined[:120]}...")    # 打印精炼后文本前 120 字
```

### 7.3 AbstractiveRefiner — 生成式摘要压缩

`AbstractiveRefiner` 使用独立的 Seq2Seq 模型（如 Flan-T5、RECOMP-abstractive）
对检索文档进行摘要/改写，生成更简洁的上下文后再传给主生成器。
适合文档噪声大、chunk 长度大（token 开销高）的场景，但需要额外加载生成模型。


```python
# ============================================================
# AbstractiveRefiner — 用 Seq2Seq 模型对检索文档生成摘要
# ============================================================
from flashrag.refiner import AbstractiveRefiner  # 生成式摘要精炼器（需独立 Seq2Seq 模型）

# Config 关键字段：
#   refiner_name       : "abstractive"，触发 AbstractiveRefiner
#   refiner_model_path : str，Seq2Seq 精炼模型路径
#   refiner_encode_max_length : int，输入最大 token 数
#
# ---- 常用 Abstractive Refiner 模型参考 ----
#   fangyuan/ance_abstractive_compressor   : RECOMP 论文官方摘要压缩模型（英文）
#   google/flan-t5-base                    : 轻量 Seq2Seq，可用于摘要，中英均可
#   ⚠ AbstractiveRefiner 需要独立生成模型，资源消耗高于 ExtractiveRefiner
#     中文场景建议优先尝试 ExtractiveRefiner，效果接近且无需额外加载 Seq2Seq 模型
config_abstractive = Config(config_path, {    # Config: 生成式精炼器专用配置
    "index_path":                actual_index_path,  # str: FAISS 索引文件路径（已构建）
    "refiner_name":              "abstractive",       # str: 启用生成式摘要精炼器
    "refiner_model_path":        "fangyuan/ance_abstractive_compressor",  # str: RECOMP 摘要模型
    "refiner_encode_max_length": 512,                 # int: 输入最大 token 数
})

# ⚠ 以下代码在模型未下载时会报错，仅演示 API 用法
# 下载模型后取消注释即可实际运行：
# abs_refiner = AbstractiveRefiner(config_abstractive)  # AbstractiveRefiner: 生成式摘要精炼器实例
# test_dataset.update_output("retrieval_result", refine_candidates)  # 写入检索结果
# abs_refined = abs_refiner.batch_run(test_dataset)   # List[str]: 摘要后的上下文列表，长度 N
# print(abs_refined[0][:200])   # 打印第一条摘要后上下文的前 200 字

print("AbstractiveRefiner 配置已准备（需下载模型后取消注释运行）")  # 打印配置就绪提示
print(f"  精炼模型: {config_abstractive['refiner_model_path']}")    # 打印精炼模型路径
```

### 7.4 LLMLinguaRefiner — Token 级细粒度压缩

`LLMLinguaRefiner` 调用 [LLMLingua](https://github.com/microsoft/LLMLingua) 库，
以小型语言模型（如 LLaMA-2-7B）计算每个 token 的条件概率，删除低信息密度的 token，
在不大幅改变文本语义的前提下实现 **2×～4× 压缩比**。

> **注意**：需要额外安装 `llmlingua` 库并下载小型 LM：
> ```bash
> pip install llmlingua
> ```
> 推荐模型：`microsoft/llmlingua-2-bert-base-multilingual-cased-meetingbank`（多语言，4 GB 以内 GPU 可运行）。


```python
# ============================================================
# LLMLinguaRefiner — Token 级细粒度 Prompt 压缩
# ============================================================
from flashrag.refiner import LLMLinguaRefiner  # Token 级精炼器（需安装 llmlingua 库）

# Config 关键字段（来自源码 LLMLinguaRefiner.__init__）：
#   refiner_name           : "llmlingua"，触发 LLMLinguaRefiner
#   refiner_model_path     : str，LLMLingua 压缩模型路径（小型 LM）
#   target_token           : int，压缩后的目标 token 数（控制压缩比）
#   dynamic_context_compression_ratio : float，动态压缩率（0~1，越小压缩越激进）
#
# ---- 常用 LLMLingua 模型参考 ----
#   microsoft/llmlingua-2-bert-base-multilingual-cased-meetingbank : 多语言，推荐，~300 MB
#   huggyllama/llama-7b                                            : 英文，效果更强，~14 GB
config_lingua = Config(config_path, {    # Config: LLMLingua token 级精炼器专用配置
    "index_path":          actual_index_path,   # str: FAISS 索引文件路径（已构建）
    "refiner_name":        "llmlingua",          # str: 启用 LLMLingua token 级压缩
    "refiner_model_path":  "microsoft/llmlingua-2-bert-base-multilingual-cased-meetingbank",  # str: LLMLingua 模型
    "target_token":        300,                   # int: 压缩目标 token 数（原始 ~1000 → 压缩至 ~300）
    "dynamic_context_compression_ratio": 0.4,     # float: 动态压缩率，0.4 = 保留约 40% 信息量
})

# ⚠ 以下代码需先安装 llmlingua 并下载模型，仅演示 API 用法
# pip install llmlingua
# 取消注释后可实际运行：
# lingua_refiner = LLMLinguaRefiner(config_lingua)          # LLMLinguaRefiner: token 级精炼器实例
# test_dataset.update_output("retrieval_result", refine_candidates)  # 写入检索结果
# lingua_refined = lingua_refiner.batch_run(test_dataset)   # List[str]: 压缩后上下文列表，长度 N
# print(f"压缩后约 {len(lingua_refined[0].split())} 词")   # 粗略统计压缩后词数

print("LLMLinguaRefiner 配置已准备（需安装 llmlingua + 下载模型后取消注释运行）")  # 打印配置就绪提示
print(f"  LLMLingua 模型: {config_lingua['refiner_model_path']}")   # 打印模型路径
print(f"  压缩目标 token 数: {config_lingua['target_token']}")       # 打印压缩目标
```

### 7.5 在自定义 Pipeline 中集成 Refiner

Refiner 在 Pipeline 中的标准接入方式：

1. `retriever.batch_search()` → 获取候选文档
2. `reranker.rerank()`（可选）→ 精排候选文档
3. `dataset.update_output("retrieval_result", ...)` → 写入检索结果供 Refiner 读取
4. `refiner.batch_run(dataset)` → 获取精炼后的上下文字符串列表
5. `prompt_template.get_string(formatted_reference=refined_text)` → 构造 Prompt（传字符串而非 List[Dict]）
6. `generator.generate()` → 生成最终回答

> **注意**：应将精炼后的 `refined_text`（`str`）传给 `formatted_reference` 参数，
> 而非传 `retrieval_result`（`List[Dict]`），避免 PromptTemplate 对已精炼的字符串重复格式化。


```python
# ============================================================
# 带 ExtractiveRefiner 的自定义 RAG Pipeline
# ============================================================
from flashrag.pipeline import BasicPipeline  # 所有 Pipeline 的基类


class RefinerRAGPipeline(BasicPipeline):  # 继承 BasicPipeline，实现「检索→精炼→生成」流程
    """
    带 ExtractiveRefiner 的自定义 RAG Pipeline。

    流程：
      1. batch_search()              粗排检索
      2. dataset.update_output()     写入检索结果供 Refiner 读取
      3. refiner.batch_run(dataset)  句子级抽取精炼，返回 List[str]
      4. get_string(formatted_reference=...) 构造 Prompt（使用精炼后字符串）
      5. generate()                  生成最终回答

    Parameters
    ----------
    config  : Config，FlashRAG 配置对象
    refiner : ExtractiveRefiner（或其他 BaseRefiner 子类），已初始化的精炼器
    """

    def __init__(self, config, refiner):   # config: Config 配置对象，refiner: BaseRefiner 精炼器
        super().__init__(config)            # 调用父类 BasicPipeline.__init__，初始化 retriever/generator/evaluator
        self.refiner = refiner              # BaseRefiner: 保存精炼器实例
    ...
    def run(self, dataset, do_eval: bool = True, pred_process_fun=None):   # dataset: Dataset 输入数据集
        ...
        Parameters
        ----------
        dataset          : Dataset，FlashRAG 格式问答数据集
        ...
        # ---- Step 2: 将检索结果写入 Dataset，供 Refiner 读取 ----
        # batch_run 内部通过 dataset.retrieval_result 属性访问检索结果，必须先写入
        dataset.update_output("retrieval_result", all_candidates)   # 写入检索结果字段（Refiner 需要）

        # ---- Step 3: 运行精炼器 ----
        # refiner.batch_run(dataset) :
        #   输入 : Dataset，需含 question 和 retrieval_result 字段
        #   返回 : List[str]，长度 N
        #     每条为精炼后的上下文字符串（ExtractiveRefiner 按句子相似度抽取 Top-K 句拼接）
        refined_texts = self.refiner.batch_run(dataset)   # List[str]: 精炼后的上下文，长度 N

        # ---- Step 4: 用精炼后文本构造 Prompt ----
        # 注意：使用 formatted_reference（str）而非 retrieval_result（List[Dict]）：
        #   refined_texts 已是格式化好的字符串，直接注入 {reference} 占位符；
        #   若改传 retrieval_result=docs，PromptTemplate 会重新格式化原始 List[Dict]，忽略精炼结果
        input_prompts = [                              # List[str | List[Dict]]: 每条问题的完整 Prompt
            self.prompt_template.get_string(
                question=q,                            # str: 问题文本，填充 {question} 占位符
                formatted_reference=ref,               # str: 精炼后上下文，直接注入 {reference} 占位符
            )
            for q, ref in zip(questions, refined_texts)   # q: str，ref: str 精炼后上下文
        ]

        # ---- Step 5: 批量生成 ----
        # generator.generate 返回 List[str]，长度 N
        preds = self.generator.generate(input_prompts)   # List[str]: 批量生成回答，长度 N

        # ---- 写回 Dataset ----
        # retrieval_result 已在 Step 2 写入，此处只补充 pred
        dataset.update_output("pred", preds)   # 写入预测回答字段

        # ---- 评估（调用父类方法）----
        # self.evaluate 来自 BasicPipeline 基类，计算 config["metrics"] 中指定的所有指标
        dataset = self.evaluate(dataset, do_eval=do_eval, pred_process_fun=pred_process_fun)   # 附加评估分数
        return dataset   # Dataset，含 pred 和 retrieval_result 字段


# ---- 使用自定义 RefinerRAGPipeline ----
refiner_pipeline_config = Config(config_path, {    # Config: 含 ExtractiveRefiner 的 Pipeline 配置
    "index_path":                actual_index_path,          # str: FAISS 索引文件路径（已构建）
    "refiner_name":              "extractive",               # str: 使用抽取式精炼器
    "refiner_model_path":        "BAAI/bge-small-zh-v1.5",   # str: 复用检索编码模型
    "refiner_input_type":        "retrieval_results",        # str: 以检索文档为精炼输入
    "refiner_topk":              5,                           # int: 保留 Top-5 相关句
    "refiner_encode_max_length": 512,                         # int: 编码最大 token 数
})

# 初始化 ExtractiveRefiner（资源消耗最低，推荐优先尝试）
pipeline_extractor = ExtractiveRefiner(refiner_pipeline_config)   # ExtractiveRefiner: 抽取式精炼器
refiner_pipeline   = RefinerRAGPipeline(refiner_pipeline_config, pipeline_extractor)   # 自定义 Pipeline 实例

print("运行 RefinerRAG Pipeline（检索 → 精炼 → 生成）...")   # 打印运行开始提示
refiner_output = refiner_pipeline.run(test_dataset, do_eval=False)   # Dataset: 含 pred 字段

print("\n--- RefinerRAG 结果 ---")   # 打印结果区标题
# 遍历输出数据集，item 含 question/pred/retrieval_result 等字段
for item in refiner_output:   # item: Dict（question/pred/retrieval_result/golden_answers）
    print(f"\n问题: {item['question']}")           # 打印问题文本
    print(f"预测: {item['pred'][:80]}...")          # 打印生成回答（前 80 字）
```


### 7.6 SelectiveContextRefiner — 基于困惑度的上下文筛选

`SelectiveContextRefiner` 使用**困惑度（Perplexity）**衡量每个句子/短语的信息密度：
困惑度高的句子（模型难以预测）通常包含更多有用信息，困惑度低的句子（模型很容易预测）通常是冗余/无信息的填充句。
精炼器保留高困惑度内容，移除低困惑度冗余内容，无需额外的生成模型。

> 来自论文：*Compressing Context to Enhance Inference Efficiency of Large Language Models*（Li et al., 2023）


```python
# ============================================================
# SelectiveContextRefiner — 基于困惑度的上下文筛选
# ⚠ 仅支持英文（源码 lang="en" 硬编码，model_type 固定为 "gpt2"）
# ============================================================
from flashrag.refiner import SelectiveContextRefiner  # 基于困惑度的上下文精炼器

# Config 关键字段（来自源码 flashrag/refiner/refiner.py）：
#   refiner_name       : str，触发 SelectiveContextRefiner 的名称
#     ✅ 合法值示例："selective-context"（包含 "selective-context" 子串）
#     ❌ 错误写法 ："selective_context"（下划线版本不匹配 get_refiner 判断条件）
#   refiner_model_path : str，GPT-2 类型模型路径（用于计算困惑度）
#     源码内部固定 model_type="gpt2"，lang="en" → 只能用英文 GPT-2 系列模型
#   sc_config          : Dict，SelectiveContextRefiner 子配置字典（⚠ 非顶层 key）
#     reduce_ratio     : float，压缩率（0~1），默认 0.5
#       0.35 → 删除困惑度最低的 35% 内容，保留 65%
#       值越大压缩越激进；通常 0.2~0.5 效果较好
config_selective = Config(config_path, {    # Config: 困惑度精炼器配置
    "index_path":           actual_index_path,  # str: FAISS 索引文件路径
    "refiner_name":         "selective-context",  # str: 触发 SelectiveContextRefiner（含 "selective-context" 子串）
    "refiner_model_path":   "gpt2",               # str: GPT-2 模型（⚠ 仅支持英文，使用 openai-community/gpt2）
    "sc_config": {                                # Dict: SelectiveContextRefiner 专属配置（非顶层 key）
        "reduce_ratio": 0.35,                     # float: 压缩率，删除 35% 低困惑度冗余内容
    },
})

# SelectiveContextRefiner(config) : 初始化困惑度精炼器
# 内部 SelectiveContext(model_type="gpt2", model_path=config["refiner_model_path"], lang="en")
# 通过 sc_config（或默认 {"reduce_ratio": 0.5}）控制压缩强度
selective_refiner = SelectiveContextRefiner(config_selective)  # SelectiveContextRefiner: 困惑度精炼器实例

print("SelectiveContextRefiner 初始化完成！")                                       # 打印初始化成功提示
print(f"  LM 模型: {config_selective['refiner_model_path']}")                       # 打印困惑度模型路径
print(f"  压缩率: {config_selective['sc_config']['reduce_ratio'] * 100:.0f}% 内容将被删除")  # 打印压缩率
```


```python
# ============================================================
# 使用 SelectiveContextRefiner 精炼检索文档
# ============================================================
# Step 1: 获取候选文档（需先完成第四章 DenseRetriever 初始化）
selective_retriever = DenseRetriever(config_selective)      # DenseRetriever: 加载 FAISS 索引
selective_candidates = selective_retriever.batch_search(    # List[List[Dict]]: 粗排候选文档
    ["RAG 技术的三个发展阶段", "FlashRAG 支持哪些生成器"]
)

# Step 2: 将检索结果写入 Dataset
test_dataset.update_output("retrieval_result", selective_candidates)  # 写入检索结果

# Step 3: 运行 SelectiveContextRefiner
# batch_run 接口与其他 Refiner 完全相同
# 返回 List[str]：每条查询的精炼后上下文（删除低困惑度冗余内容后的文本）
selective_refined = selective_refiner.batch_run(test_dataset)  # List[str]: 精炼后的上下文，长度 N

print("SelectiveContextRefiner 精炼结果:")   # 打印结果标题
for i, refined in enumerate(selective_refined, 1):  # 遍历精炼后文本
    print(f"  查询 {i} 精炼后（前 120 字）: {refined[:120]}...")   # 打印精炼后文本前缀
```

## 八、生成器

### 8.1 OpenaiGenerator — OpenAI 兼容 API 生成器
...
### 8.2 HFCausalLMGenerator — 本地 HuggingFace 模型生成器
...
### 8.3 PromptTemplate — 自定义 Prompt 模板
...
| `ConditionalPipeline` | `pipeline.py` | 判断是否需要检索（Judger）|
| `FLAREPipeline` | `active_pipeline.py` | 前向迭代检索 |
| `SelfRAGPipeline` | `active_pipeline.py` | 自适应反思检索（需专用模型）|
...
| `SuRePipeline` | `branching_pipeline.py` | 验证后选优（SuRe）|
| `SelfAskPipeline` | `active_pipeline.py` | 子问题分解+迭代检索 |
| `AdaptivePipeline` | `pipeline.py` | 自适应检索策略（Adaptive-RAG）|
| `ReasoningPipeline` | `reasoning_pipeline.py` | 多步推理检索（Think + Search）|

> 注意：
> - `HyDEPipeline` 不存在，HyDE 通过自定义 Pipeline 实现（见 9.4 节）
> - `SKRPipeline` 不存在，SKR 算法通过 `ConditionalPipeline` + `SKRJudger` 实现（见 9.6 节）
> - `AdaptiveRAGPipeline` 不存在，Adaptive-RAG 通过 `AdaptivePipeline` 实现（见 9.9 节）
...
**SelfAskPipeline — 子问题分解检索（Self-Ask）**

**流程**：生成器将复杂问题拆解为 "Follow up:" 格式的子问题 → 对每个子问题独立检索 → 逐步拼接中间答案 → 最终生成 "So the final answer is:" 的汇总回答。

- **优点**：能分解多跳问题为可独立回答的子问题，每个子问题都有独立检索支撑；无需特殊模型
- **缺点**：子问题数量不固定，总延迟难以预测；对生成模型的指令跟随能力要求较高
- **适用场景**：多跳事实性问答（如"A 的老师毕业于哪所大学？"）

---

**SKR — 选择性知识检索（Selective Knowledge Retrieval）**

> 实现方式：`ConditionalPipeline` + `SKRJudger`（`SKRPipeline` 类不存在）

**流程**：`SKRJudger` 基于 KNN 在预标注训练集中寻找相似 query，通过投票判断是否需要检索 → `ConditionalPipeline` 根据判断结果走 RAG 或直接生成。

- **优点**：判断逻辑轻量（纯 KNN 推理，无需生成模型参与判断）
- **缺点**：需要预先标注大量 (query, 是否需要检索) 训练数据；KNN 检索有一定额外开销
- **适用场景**：有标注预算、知识分布差异明显的问答任务（见 9.9 节代码示例）

---

**AdaptivePipeline — 自适应检索（Adaptive-RAG）**

**流程**：`AdaptiveJudger`（T5 序列到序列分类器）判断 query 复杂度输出 A/B/C → A（无需检索，直接生成）/ B（单跳 SequentialPipeline）/ C（多跳 IRCOTPipeline）。

- **优点**：以最合适的策略回答不同复杂度的问题，兼顾效率与质量
- **缺点**：需要预训练的 query 复杂度分类器（seq2seq 模型）；三条 Pipeline 共享 retriever/generator 实例
- **适用场景**：混合复杂度问答数据集（如 2WikiMultiHopQA、MuSiQue、PopQA）

---

**SuRePipeline — 验证后选优（SuRe）**

**流程（四步）**：
1. **候选生成**：对每篇检索文档分别生成候选答案
2. **摘要生成**：为每篇文档生成支持声明摘要
3. **成对验证**：对每条 (摘要, 候选答案) 对进行 True/False 验证（判断摘要是否支持答案）
4. **成对排序**：对验证为 True 的候选答案进行两两比较排序，选出最优答案

- **优点**：通过多轮显式验证大幅降低幻觉；相比 REPLUG 更注重答案可靠性而非概率融合
- **缺点**：推理开销 = retrieval_topk × 候选生成 + retrieval_topk × 验证 + 排序；延迟极高
- **适用场景**：对答案准确性要求极高、可接受较高延迟的离线评测场景

### 9.1 准备 Pipeline 输入数据集
...
### 9.2 SequentialPipeline — Naive RAG
...
### 9.6 ConditionalPipeline — 自适应检索判断


```python
# ============================================================
# ConditionalPipeline — 使用 SKRJudger 判断每条 query 是否需要检索
# ============================================================
from flashrag.pipeline import ConditionalPipeline  # 条件检索 Pipeline（含 Judger 判断模块）

# ConditionalPipeline 需要一个 Judger 组件：
#   Judger 对每条 query 输出 True（需要检索）或 False（直接生成）
#   由 flashrag.utils.get_judger(config) 根据 judger_name 自动实例化（来自 utils.py 源码）：
#     judger_name 含 "skr"      → SKRJudger  （KNN 投票判断，需要预标注训练数据）
#     judger_name 含 "adaptive" → AdaptiveJudger（T5 分类器，见 9.9 节 AdaptivePipeline）
#
# ---- ConditionalPipeline Config 关键字段（来自源码）----
#   judger_name   : str，决定实例化哪个 Judger 类（必填）
#   judger_config : Dict，Judger 类的所有参数（嵌套字典，⚠ 非顶层 key）
#     SKRJudger 需要的 judger_config 字段：
#       model_path        : str，计算 query 嵌入的编码器模型路径
#       training_data_path: str，预标注训练数据 JSON 路径（每条含 question + judgement 字段）
#                           judgement 的合法值："ir_better"（需要检索）/ "ir_worse"（不需要）
#       topk              : int，KNN 邻居数（默认 5）
#       batch_size        : int，编码批次大小（默认 64）
#       max_length        : int，编码最大 token 数（默认 128）
config_conditional = Config(config_path, {     # Config: ConditionalPipeline 配置
    "index_path":  actual_index_path,          # str: FAISS 索引文件路径（已构建）
    "judger_name": "skr",                      # str: 使用 SKRJudger（名称需含 "skr"，大小写不敏感）
    "judger_config": {                         # Dict: SKRJudger 专属配置（⚠ 嵌套字典，非顶层 key）
        "model_path":         "BAAI/bge-small-zh-v1.5",  # str: query 编码器模型（可复用检索编码器）
        "training_data_path": "path/to/skr_training.json",  # str: ⚠ 需自备或下载官方 SKR 标注数据
        # 标注格式：[{"question": "...", "judgement": "ir_better"/"ir_worse"}, ...]
        "topk":       5,    # int: KNN 邻居数（投票样本数，默认 5）
        "batch_size": 64,   # int: 编码批次大小
        "max_length": 128,  # int: 编码最大 token 数
    },
})

# ConditionalPipeline(config) : 初始化，内部自动创建 SKRJudger、retriever、generator
# ⚠ 需要先准备好 skr_training.json（已标注 query 是否需要检索的 JSON 文件）
conditional_pipeline = ConditionalPipeline(config_conditional)  # ConditionalPipeline 实例

print("运行 ConditionalPipeline（SKRJudger 自动判断是否需要检索）...")  # 打印运行开始提示
conditional_output = conditional_pipeline.run(test_dataset, do_eval=False)  # Dataset: 含 pred 字段

print("\n--- ConditionalPipeline 结果 ---")   # 打印结果区标题
for item in conditional_output:   # item: Dict（question/pred/golden_answers）
    print(f"  Q: {item['question']}")          # 打印问题
    print(f"  A: {item['pred'][:80]}...")       # 打印生成回答（前 80 字）
```

### 9.7 SelfRAGPipeline — 自我反思检索（Self-RAG）


```python
# ============================================================
# SelfRAGPipeline — 使用反思 token 自主决定检索策略
# ============================================================
from flashrag.pipeline import SelfRAGPipeline  # 自我反思检索 Pipeline（需专用 fine-tune 模型）

# ⚠ 重要：Self-RAG 必须使用专门 fine-tune 过的 Self-RAG 模型，通用 LLM 无法使用！
# 推荐模型（HuggingFace Hub）：
#   selfrag/selfrag_llama2_7b  （7B，英文，~14 GB）
#   selfrag/selfrag_llama2_13b （13B，英文，~26 GB）
#
# Self-RAG 模型生成时会输出特殊反思 token（Reflection Tokens，来自源码类属性）：
#   检索类 : [Retrieval]  / [No Retrieval]  / [Continue to Use Evidence]
#   相关类 : [Relevant]   / [Irrelevant]
#   支持类 : [Fully supported] / [Partially supported] / [No support / Contradictory]
#   效用类 : [Utility:1] ~ [Utility:5]
#
# ⚠ SelfRAGPipeline 的检索策略参数是构造器参数（来自源码 __init__ 签名），
#    不是 Config 字典的 key！Config 中只需填写模型路径和通用生成参数。
#
# Config 关键字段（通用参数，非 Self-RAG 特有）：
#   index_path           : str，FAISS 索引路径
#   framework            : str，生成后端 ("hf" / "vllm")
#   generator_model_path : str，Self-RAG 专用模型路径
config_selfrag = Config(config_path, {      # Config: SelfRAGPipeline 配置
    "index_path":            actual_index_path,       # str: FAISS 索引文件路径（已构建）
    "framework":             "hf",                    # str: 本地 HuggingFace 模型后端
    "generator_model_path":  "selfrag/selfrag_llama2_7b",  # str: Self-RAG 专用模型路径
    "generation_params": {                            # Dict: 生成超参数（通用字段）
        "max_new_tokens": 512,                        # int: 最大生成 token 数
        "do_sample":      False,                      # bool: 贪婪解码（Self-RAG 通常不采样）
    },
})

# SelfRAGPipeline 构造器参数（来自源码，⚠ 不是 Config key）：
#   mode          : str，检索模式（默认 "adaptive_retrieval"）
#     "adaptive_retrieval" → 根据 [Retrieval] token 动态决定（论文默认设置，推荐）
#     "always_retrieve"    → 每步都检索（用于消融实验）
#     "no_retrieval"       → 从不检索（用于消融实验）
#   threshold     : float，触发检索的 logprob 阈值（默认 0.2）
#   max_depth     : int，segment-level beam search 的最大深度（默认 2）
#   beam_width    : int，beam search 的宽度（默认 2）
#   w_rel/w_sup/w_use : float，[Relevant]/[Supported]/[Utility] token 的得分权重（默认均 1.0）

# ⚠ 以下代码需先下载 selfrag_llama2_7b（~14 GB），仅演示 API 用法
# 取消注释后可实际运行（确保 GPU 显存 ≥ 16 GB）：
# selfrag_pipeline = SelfRAGPipeline(   # SelfRAGPipeline 实例
#     config_selfrag,
#     mode="adaptive_retrieval",        # str: 检索策略（构造器参数，非 Config key）
#     threshold=0.2,                    # float: 触发检索的 logprob 阈值（构造器参数）
#     max_depth=2,                      # int: beam search 最大深度
#     beam_width=2,                     # int: beam search 宽度
#     w_rel=1.0, w_sup=1.0, w_use=1.0, # float: 三类反思 token 的得分权重
# )
# selfrag_output = selfrag_pipeline.run(test_dataset, do_eval=False)  # Dataset: 含 pred 字段
# for item in selfrag_output:
#     print(f"Q: {item['question']}\nA: {item['pred'][:80]}...")

print("SelfRAGPipeline 配置已准备")                              # 打印配置就绪提示
print("⚠ 需要下载 selfrag_llama2_7b（~14 GB）后取消注释运行")   # 打印模型下载提示
print("  mode=adaptive_retrieval, threshold=0.2")               # 打印默认策略参数
```

### 9.8 SelfAskPipeline — 子问题分解检索（Self-Ask）


```python
# ============================================================
# SelfAskPipeline — 子问题分解 + 迭代检索
# ============================================================
from flashrag.pipeline import SelfAskPipeline  # 子问题分解 Pipeline（查询转换）

# Self-Ask 的核心思想：
#   1. 生成器将复杂问题分解为 "Follow up: <子问题>" 格式的简单问题序列
#   2. 对每个子问题独立执行「检索 → 生成子答案」
#   3. 将所有子答案拼接作为上下文，生成最终回答（"So the final answer is: ..."）
# 特点：
#   - 无需特殊模型，依赖通用 LLM 的指令跟随能力
#   - 适合需要多步推理的多跳问题
#   - 子问题数量由生成器动态决定，不固定

config_selfask = Config(config_path, {    # Config: SelfAskPipeline 配置
    "index_path":   actual_index_path,   # str: FAISS 索引文件路径（已构建）
    # Self-Ask 不需要额外特殊配置，使用通用生成器
    # 可通过 generation_params 调整生成行为
})

# SelfAskPipeline 构造器参数（来自源码 __init__ 签名，⚠ 不是 Config key）：
#   max_iter   : int，最大子问题迭代轮数（默认 5），超过后停止分解
#   single_hop : bool，使用单跳提示模板（True，默认）或多跳提示模板（False）
#     True  → SELF_ASK_PROMPT_SINGLE_HOP（每个子问题独立处理）
#     False → SELF_ASK_PROMPT_MULTI_HOP （带上下文历史的多跳处理）
selfask_pipeline = SelfAskPipeline(   # SelfAskPipeline 实例
    config_selfask,
    max_iter=5,       # int: 最多分解 5 个子问题（超出后停止）
    single_hop=True,  # bool: True → 单跳模板（推荐），False → 多跳模板
)

print("运行 SelfAskPipeline（子问题分解 + 迭代检索）...")    # 打印运行开始提示
selfask_output = selfask_pipeline.run(test_dataset, do_eval=False)  # Dataset: 含 pred 字段

print("\n--- SelfAsk 结果 ---")   # 打印结果区标题
for item in selfask_output:   # item: Dict（question/pred/golden_answers）
    print(f"  Q: {item['question']}")           # 打印问题
    print(f"  A: {item['pred'][:80]}...")        # 打印生成回答（前 80 字）
```

### 9.9 SKR + AdaptivePipeline — 选择性知识检索与自适应检索


```python
# ============================================================
# SKR — 选择性知识检索（通过 ConditionalPipeline + SKRJudger 实现）
# ⚠ FlashRAG 中没有 SKRPipeline 类；SKR 算法 = ConditionalPipeline + judger_name="skr"
# ============================================================
from flashrag.pipeline import ConditionalPipeline  # 条件检索 Pipeline（通用 Judger 框架）

# SKRJudger 算法（来自源码 flashrag/judger/judger.py）：
#   使用 KNN 检索预标注训练集，通过投票判断当前 query 是否需要检索
#   训练数据格式（JSON）：[{"question": "...", "judgement": "ir_better" or "ir_worse"}, ...]
#     "ir_better" → 检索比不检索效果更好，应触发检索
#     "ir_worse"  → 检索无益甚至有害，应直接生成
#   判断逻辑：找到 topk 个最相似的训练样本，用 ir_better vs ir_worse 的比例投票
#
# ---- SKR Config 关键字段（来自源码 SKRJudger.__init__）----
#   judger_name          : str，名称需含 "skr"（大小写不敏感，来自 get_judger 判断）
#   judger_config        : Dict，SKRJudger 专属配置（嵌套字典，⚠ 非顶层 key）
#     model_path         : str，query 编码器模型（用于 KNN 相似度计算）
#     training_data_path : str，标注训练数据 JSON 路径（必须预先准备）
#     topk               : int，KNN 邻居数（默认 5）
#     batch_size         : int，编码批次大小（默认 64）
#     max_length         : int，编码最大 token 数（默认 128）
config_skr = Config(config_path, {    # Config: SKR（ConditionalPipeline + SKRJudger）配置
    "index_path":  actual_index_path, # str: FAISS 索引文件路径（已构建）
    "judger_name": "skr",             # str: 触发 SKRJudger（名称含 "skr"）
    "judger_config": {                # Dict: SKRJudger 专属配置（⚠ 嵌套字典）
        "model_path":         "BAAI/bge-small-zh-v1.5",   # str: 编码器（复用检索编码器即可）
        "training_data_path": "path/to/skr_training.json",  # str: ⚠ 预标注训练数据路径（必填）
        "topk":       5,    # int: KNN 邻居数
        "batch_size": 64,   # int: 编码批次大小
        "max_length": 128,  # int: 编码最大 token 数
    },
...
# SKR = ConditionalPipeline（SKRJudger 自动判断是否需要检索）
# ⚠ 需先准备好 skr_training.json 标注文件，替换 training_data_path 后运行
skr_via_conditional = ConditionalPipeline(config_skr)   # ConditionalPipeline: SKR 实现实例
# skr_output = skr_via_conditional.run(test_dataset, do_eval=False)  # Dataset: 含 pred 字段

print("SKR（ConditionalPipeline + SKRJudger）配置已准备")              # 打印配置就绪提示
print("⚠ 需要预先准备 skr_training.json（带 judgement 标注的训练数据）")  # 打印注意事项
```


```python
# ============================================================
# AdaptivePipeline — 自适应检索策略（Adaptive-RAG）
# ⚠ 类名为 AdaptivePipeline（来自 pipeline.py），不是 AdaptiveRAGPipeline
# ============================================================
from flashrag.pipeline import AdaptivePipeline  # 自适应检索 Pipeline（来自 pipeline.py）

# AdaptivePipeline 流程（来自源码 AdaptivePipeline.run，论文 "Adaptive-RAG"）：
#   Step 1: AdaptiveJudger（T5 seq2seq 分类器）对每条 query 输出 A/B/C
#             "A" → 无需检索，SequentialPipeline.naive_run（直接生成）
#             "B" → 单跳检索，SequentialPipeline.run
#             "C" → 多跳检索，IRCOTPipeline.run（max_iter=5）
#   Step 2: 根据分类结果将数据集切分并路由至对应 Pipeline
#   Step 3: 合并各分支结果，返回统一格式 Dataset
#
# ---- AdaptivePipeline Config 关键字段（来自源码 AdaptivePipeline.__init__ + get_judger）----
#   judger_name    : str，名称需含 "adaptive"（触发 AdaptiveJudger，大小写不敏感）
#   judger_config  : Dict，AdaptiveJudger 专属配置（嵌套字典，⚠ 非顶层 key）
#     model_path   : str，Adaptive-RAG 分类器模型路径（Seq2Seq T5 类型）
#                   官方预训练模型：参见 FlashRAG GitHub 配套资源
#     batch_size   : int，分类器批次大小（默认 16）
#     max_length   : int，分类器最大输入 token 数（默认 512）
config_adaptive = Config(config_path, {    # Config: AdaptivePipeline 配置
    "index_path":  actual_index_path,      # str: FAISS 索引文件路径（已构建）
    "judger_name": "adaptive",             # str: 触发 AdaptiveJudger（名称含 "adaptive"）
    "judger_config": {                     # Dict: AdaptiveJudger 专属配置（⚠ 嵌套字典）
        "model_path": "path/to/adaptive_rag_classifier",  # str: ⚠ 替换为实际分类器路径
        # 官方分类器可通过 FlashRAG GitHub 配套资源下载
        "batch_size": 16,    # int: 分类器批次大小
        "max_length": 512,   # int: 分类器最大输入 token 数
    },
})

# AdaptivePipeline(config) : 初始化，内部自动创建 AdaptiveJudger + 三条子 Pipeline（norag/single/multi）
# ⚠ 以下代码需要预训练的 Adaptive-RAG 分类器，取消注释并替换路径后运行
# adaptive_pipeline = AdaptivePipeline(config_adaptive)             # AdaptivePipeline 实例
# adaptive_output   = adaptive_pipeline.run(test_dataset, do_eval=False)  # Dataset: 含 pred 字段
# for item in adaptive_output:
#     print(f"Q: {item['question']}\n路由: {item['judge_result']}\nA: {item['pred'][:60]}...")

print("AdaptivePipeline 配置已准备（⚠ 类名为 AdaptivePipeline，非 AdaptiveRAGPipeline）")  # 打印说明
print("⚠ 需要预训练 Adaptive-RAG 分类器，替换 judger_config['model_path'] 后取消注释运行")   # 打印注意事项
```


### 9.10 SuRePipeline — 验证后选优（SuRe）


```python
# ============================================================
# SuRePipeline — 多步候选生成 + 验证排序（SuRe）
# ============================================================
from flashrag.pipeline import SuRePipeline  # SuRe Pipeline（来自 branching_pipeline.py）

# SuRePipeline 四步流程（来自源码 branching_pipeline.py）：
#   Step 1 候选生成 : 对每篇检索文档（共 retrieval_topk 篇）独立生成候选答案
#   Step 2 摘要生成 : 为每篇文档生成 "supporting passage" 摘要声明
#   Step 3 验证     : 对每对 (摘要, 候选答案) 判断摘要是否支持答案（True / False）
#   Step 4 排序     : 对验证为 True 的候选答案两两比较（Passage 1 / Passage 2），
#                   选出最终最优答案
# 全程使用同一个 generator，但调用次数 ≈ topk × 3 + 排序次数（延迟较高）
#
# SuRePipeline 只需标准配置，无特殊参数（来自源码 __init__ 签名）：
#   SuRePipeline(config, prompt_template=None, retriever=None, generator=None)
config_sure = Config(config_path, {     # Config: SuRePipeline 配置
    "index_path":       actual_index_path,  # str: FAISS 索引文件路径（已构建）
    "retrieval_topk":   5,                  # int: 候选文档数（同时也是候选答案数）
    # SuRePipeline 无特殊配置字段，使用与 SequentialPipeline 相同的通用字段
})

# SuRePipeline(config) : 初始化，内部自动创建 retriever、generator 和四套 PromptTemplate
# 四套 Prompt（来自源码）：
#   P_CAN_TEMPLATE : 候选答案生成 Prompt（输入 passages + question → (a) xx, (b) yy...）
#   P_SUM_TEMPLATE : 摘要生成 Prompt（输入 prediction + passage → supporting statement）
#   P_VAL_TEMPLATE : 验证 Prompt（输入 prediction + summary → True/False）
#   P_RANK_TEMPLATE: 排序 Prompt（输入两段摘要 + question → Passage 1 / Passage 2）
sure_pipeline = SuRePipeline(config_sure)   # SuRePipeline 实例

print("运行 SuRePipeline（四步候选生成 + 验证排序）...")       # 打印运行开始提示
sure_output = sure_pipeline.run(test_dataset, do_eval=False)  # Dataset: 含 pred 字段

print("\n--- SuRe 结果 ---")   # 打印结果区标题
for item in sure_output:   # item: Dict（question/pred/golden_answers）
    print(f"  Q: {item['question']}")          # 打印问题
    print(f"  A: {item['pred'][:80]}...")       # 打印验证排序后的最优答案（前 80 字）
```
...
将前十章所有组件串联为 6 个函数，清晰展示各步骤的拼接方式：
...
| 7.2 | 抽取式精炼 | `ExtractiveRefiner(config).batch_run(dataset)` → `List[str]` |
| 7.3 | 生成式精炼 | `AbstractiveRefiner(config).batch_run(dataset)` → `List[str]` |
| 7.4 | Token 级压缩 | `LLMLinguaRefiner(config).batch_run(dataset)` → `List[str]` |
| 8.1 | API 生成 | `OpenaiGenerator(config).generate(input_list)` |
| 8.2 | 本地生成 | `get_generator(config)` → `HFCausalLMGenerator` |
| 8.3 | Prompt 模板 | `PromptTemplate(config).get_string(question=q, retrieval_result=r)` |
| 9.2 | Naive RAG | `SequentialPipeline(config).run(dataset)` |
| 9.3 | FLARE | `FLAREPipeline(config).run(dataset)`（需开启 logprobs） |
| 9.4 | HyDE（手动） | `generate(hypo_prompt)` → `batch_search(hypo_doc)` |
| 9.5 | 自定义 Pipeline | 继承 `BasicPipeline`，重写 `run()` |
| 9.6 | ConditionalPipeline | `ConditionalPipeline(config).run(dataset)`（含 Judger）|
| 9.7 | Self-RAG | `SelfRAGPipeline(config).run(dataset)`（需 fine-tune 模型）|
| 9.8 | Self-Ask | `SelfAskPipeline(config).run(dataset)` |
| 9.9 | SKR + AdaptivePipeline | `ConditionalPipeline`（SKRJudger）/ `AdaptivePipeline` |
| 9.10 | SuRePipeline | `SuRePipeline(config).run(dataset)` |
| 10 | 评估 | `Evaluator(config).evaluate(dataset)` |
| **11** | **端到端** | **step1→step6 串联，一键跑通** |
### RAG 系统建设核心清单
...
□ 6. 精炼器  ：ExtractiveRefiner / SelectiveContextRefiner.batch_run(dataset) → List[str]（可选）
□ 7. 生成后端：API（OpenaiGenerator）/ 本地（get_generator() → HFCausalLMGenerator）
□ 8. Pipeline ：SequentialPipeline（基础）/ FLAREPipeline / SelfAskPipeline（子问题分解）
□            或 ConditionalPipeline（含 SKRJudger）/ AdaptivePipeline（自适应）/ 自定义继承 BasicPipeline
□ 9. 检索源  ：本地索引（Dense/BM25）/ 混合检索（MultiRetrieverRouter）/ 网络搜索（WebSearchRetriever）
□ 10. 评估指标：Evaluator（EM/F1/acc，来自 basic_config.yaml 默认 metrics）
```
